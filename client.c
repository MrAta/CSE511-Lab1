...
void *client_func() {
  ...
  while (local_count <= (int)(NUM_OPS/NUM_THREADS)) {
    ...
    double time_taken;
    if(rv == 0)
    time_taken = (double) (rtve.tv_usec - rtvs.tv_usec) / 1000000 + (double) (rtve.tv_sec - 10 - rtvs.tv_sec);
    else
    time_taken = (double) (rtve.tv_usec - rtvs.tv_usec) / 1000000 + (double) (rtve.tv_sec - rtvs.tv_sec);
